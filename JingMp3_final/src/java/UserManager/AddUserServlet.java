...
public class AddUserServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            boolean isTaken = userDAO.isUsernameOrEmailTaken(username, email);

            if (isTaken) {
                // Username or email already exists, show an error message
                request.setAttribute("errorMessage", "Tên đăng nhập hoặc email đã được sử dụng.");
                request.getRequestDispatcher("addUser.jsp").forward(request, response);
            } else {
                // Add the new user
                User user = new User(username, email, password);
                userDAO.addUser(user);
                response.sendRedirect("manageUsers"); // Redirect back to the user management page
            }
